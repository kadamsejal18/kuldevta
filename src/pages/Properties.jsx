...
import { Search, Filter, X, SlidersHorizontal, Grid3X3, List, MapPin } from 'lucide-react'
...
export default function Properties() {
  ...
  const filtered = useMemo(() => {
    ...
    return result
  }, [search, type, category, furnishing, priceRange, sortBy])
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex gap-2">
              ...
              <div className="hidden sm:flex gap-1 glass rounded-xl p-1">
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white/10' : ''}`}
                >
                  <Grid3X3 size={16} className={viewMode === 'grid' ? 'text-white' : 'text-white/40'} />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-white/10' : ''}`}
                >
            ...
            >
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                <div className="space-y-1.5">
                  ...
                  <select value={type} onChange={(e) => setType(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white outline-none">
                ...
                <div className="space-y-1.5">
                  ...
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white outline-none">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs text-white/40">Furnishing</label>
                  <select value={furnishing} onChange={(e) => setFurnishing(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white outline-none">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs text-white/40">Price Range</label>
                  <select value={priceRange} onChange={(e) => setPriceRange(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white outline-none">
                    <option value="all" className="bg-gray-900">Any Price</option>
                    <option value="under50L" className="bg-gray-900">Under ₹50L</option>
                    <option value="50L-1Cr" className="bg-gray-900">₹50L - ₹1Cr</option>
                    <option value="1Cr-5Cr" className="bg-gray-900">₹1Cr - ₹5Cr</option>
                    <option value="above5Cr" className="bg-gray-900">Above ₹5Cr</option>
                  </select>
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs text-white/40">Sort By</label>
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white outline-none">
              ...
              {activeFilters > 0 && (
                <button onClick={clearFilters} className="mt-3 text-sm text-amber-400 hover:text-amber-300 flex items-center gap-1">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <button onClick={clearFilters} className="btn-luxury mt-4 text-sm">
              Clear Filters
            </button>
          </div>
        ) : (
